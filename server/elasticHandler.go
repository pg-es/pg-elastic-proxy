...
// ElasticEndpointRequestHandler is a handler function for requests with URL: /<index>/<type>/<endpoint>.
// Index, type and endpoint are automatically extracted from URL.
...
// ElasticHandler contains information how to process all routes of the server.
...
// HandleFuncEndpoint adds a handler for request with URL: /<index>/<type>/<endpoint>.
// Endpoint format is described via pattern.
...
// writeOutput prints output structure in JSON format to ResponseWriter.
func (h *ElasticHandler) writeOutput(w http.ResponseWriter, r *http.Request, output any) {
	var err error
	var data []byte //nolint:wsl_v5 // both declarations are related to the same marshaling logic
...
// processRequestOutput processes output of request processing with respect to errors.
...
// supportMethod checks if target method is presented in supportedMethods slice.
